...
class FedAggregator_RBLA(AbstractFedAggregator):
    ...
    def __init__(self, args: FedAggregatorArgs | None = None):
        ...
        self._pre_canonicalization_weight = None
        return
    ...
    def _after_aggregation(self) -> None:
        # Optional read-only observation point. Canonicalization returns new
        # factors, so retaining this reference needs no extra tensor copy.
        self._pre_canonicalization_weight = (
            self._aggregated_weight
            if self.args.get("retain_pre_canonicalization", False) else None
        )
        self._canonicalization_applied_last_round = False
    ...
    @property
    def pre_canonicalization_weight(self):
        """Current round's aggregate before spectral processing; read-only.

        Available when aggregation.retain_pre_canonicalization is enabled.
        Consumers must not modify the mapping or its tensors in place.
        """
        return self._pre_canonicalization_weight
