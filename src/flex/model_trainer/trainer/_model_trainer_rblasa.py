...
before each local training round via :meth:`set_rblasa_context`.
"""
...
from ...ml_algorithms.rblasa import rblasaAlignmentLoss, rblasaConfig
from ...ml_algorithms.rblasa.alignment import collect_lora_pairs


class ModelTrainer_rblasa(ModelTrainer):
    """
    Trainer that adds rblasa anti-drift alignment regularization.

    Usage (set by client strategy before each round)::

        trainer.set_rblasa_context(
            global_anchors=global_state_dict,  # server's A_g, B_g
            round_idx=round_idx,
            r_i=local_rank,
            rblasa_alignment=rblasa_module,
        )
    """
    ...
    def __init__(self, trainer_args: ModelTrainerArgs):
        ...
        # ── rblasa context (set by client strategy) ──
        self._rblasa_alignment: Optional[rblasaAlignmentLoss] = None
    ...
    # ------------------------------------------------------------------
    # rblasa context (called by client strategy before each round)
    # ------------------------------------------------------------------
    def set_rblasa_context(
        self,
        global_anchors: Dict[str, torch.Tensor],
        round_idx: int,
        r_i: int,
        rblasa_alignment: rblasaAlignmentLoss,
    ) -> None:
        ...
        self._r_i = r_i
        self._rblasa_alignment = rblasa_alignment
    ...
    def _compute_alignment_loss(self) -> Tuple[torch.Tensor, float, float]:
        ...
        if self._rblasa_alignment is None or self._global_anchors is None:
        ...
        for prefix in (self._lora_prefixes or []):
            ...
            A_glo = self._global_anchors[key_A][: self._r_i, :]  # prefix
            B_glo = self._global_anchors[key_B][:, : self._r_i]  # prefix

            _, slot, sub = self._rblasa_alignment(
                B_loc, A_loc, B_glo, A_glo, self._r_i, self._round_idx,
            )
        ...
        if count == 0:
            return torch.tensor(0.0, device=self.device), 0.0, 0.0
        return (
            self._rblasa_alignment.get_lambdas(self._round_idx)[0] * total_slot
            + self._rblasa_alignment.get_lambdas(self._round_idx)[1] * total_sub,
    ...
    def train_step(self) -> Dict[str, Any]:
        """Standard training step + rblasa alignment regularisation."""
        ...
        if self._rblasa_alignment is not None:
            lam_slot, lam_sub = self._rblasa_alignment.get_lambdas(self._round_idx)

        loop = pbar(
            train_dl,
            desc=f"rblasa train (epoch {self._epoch_idx}{'/' + str(total_epochs) if total_epochs else ''})",
        ...
        for inputs, labels in loop:
            ...
            # ── rblasa alignment regularisation ──
            align_loss = torch.tensor(0.0, device=self.device)
            if self._rblasa_alignment is not None and self._global_anchors is not None:
                ...
                for prefix in (self._lora_prefixes or []):
                    ...
                    B_loc = param_map[kB]
                    A_glo = self._global_anchors[kA][: self._r_i, :]
                    B_glo = self._global_anchors[kB][:, : self._r_i]
                    slot_loss = self._rblasa_alignment.compute_slot_loss(
                        B_loc, A_loc, B_glo, A_glo)
                    sub_loss = self._rblasa_alignment.compute_subspace_loss(
                        B_loc, B_glo, self._r_i, self._round_idx)
        ...
        tqdm_write(
            f"[rblasa Epoch {self._epoch_idx}{'/' + str(total_epochs) if total_epochs else ''} Finished] "
