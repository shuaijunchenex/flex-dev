"""
FedGRA-specific trainer — extends standard trainer with _eval_initial_loss.

FedGRA client selection uses the pre-training "surprise" loss to compute
EWM/GRA scores.  This trainer computes that metric before local training;
other strategies should use the standard trainer to avoid the overhead.
"""
...
class ModelTrainer_FedGRA(ModelTrainer_Standard):
    """Standard trainer + pre-training initial-loss evaluation for FedGRA."""

    # ------------------------------------------------------------------
    # Override: compute initial loss before local training
    # ------------------------------------------------------------------
    def train(self, epochs: int) -> Any:
        ...
        # Evaluate initial loss (pre-training) — FedGRA needs this for
        # EWM / GRA client selection.
    ...
    # ------------------------------------------------------------------
    # FedGRA-specific: evaluate model loss on local data before training
    # ------------------------------------------------------------------
    def _eval_initial_loss(self) -> float:
        """Compute loss of current (global) model on local training data
        WITHOUT any gradient update.

        This gives the 'surprise' of the global model on each client's
        local data, which has high cross-client variance.  FedGRA uses it
        for EWM / GRA client selection.
        """
        ...
        with torch.no_grad():
            dl = ta.train_loader.data_loader
            try:
                from ...ml_utils.tqdm_utils import pbar as _tqdm
                dl = _tqdm(
                    dl, desc="  Eval initial loss (FedGRA)", leave=False,
                    ncols=100,
