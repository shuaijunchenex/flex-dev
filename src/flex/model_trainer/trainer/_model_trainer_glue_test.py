"""
ModelTrainer_GlueTest
----------------------
A minimal GLUE-style trainer that processes ONLY ONE sample (one batch
truncated to a single item) per training step.  Intended for end-to-end
pipeline verification of NLP tasks (TinyBERT / RoBERTa, etc.) without
waiting for full dataset training.

Usage
-----
Set ``trainer_type: glue_test`` in your YAML, e.g.:

    trainer:
      trainer_type: glue_test
      device: cpu
...
class ModelTrainer_GlueTest(ModelTrainer):
    """
    Pipeline-test trainer for GLUE-style NLP tasks.

    Runs exactly **one sample** (the very first batch from the DataLoader,
    truncated to a single item) per training step.  Handles both plain-tensor
    inputs and dict-style inputs (``input_ids``, ``attention_mask``, etc.)
    produced by Hugging Face tokenisers.

    All return values are structurally identical to ``ModelTrainer_GLUE``
    ...
    def _truncate_to_one(self, obj):
        """Truncate batch to a single sample."""
        if isinstance(obj, torch.Tensor):
            return obj[:1]
        if isinstance(obj, dict):
            return {k: self._truncate_to_one(v) for k, v in obj.items()}
        return obj
    ...
    def train_step(self) -> Dict[str, Any]:
        ...
        console.info(
            f"[GlueTest] Epoch {self._epoch_idx}"
        ...
        inputs = self._truncate_to_one(inputs)
        ...
        # Handle HuggingFace-style output dict
        ...
        console.info(
            f"[GlueTest] Epoch {self._epoch_idx} done — "
