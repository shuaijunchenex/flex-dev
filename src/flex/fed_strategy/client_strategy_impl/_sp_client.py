...
class SpClientTrainingStrategy(ClientStrategy):
    ...
    def set_local_weight(self) -> dict:
        ...
        model = self._obj.node_var.model
        rank_dict = LoRAUtils.get_lora_ranks(model)
        lora_cfg = getattr(model, "lora_config", None) or {}
        cache = self._obj.node_var.cache_weight

        if not LoRAUtils.has_factored_keys(cache):
            # Fallback: broadcast weight is a plain state_dict (initial broadcast
            # or non-SP-aggregated round).  Slice global LoRA to local rank to
            # handle heterogeneous rank_ratio across clients.
            self._obj.node_var.model_weight = FedAggregator_RBLA.broadcast_lora_state_dict(
                cache, self._obj.node_var.model_weight
            )
            return

        local_weight = LoRAUtils.materialize_lora_from_factors(
            cache,
            rank_dict,
            lora_suffix_A=lora_cfg.get("suffix_A", "lora_A"),
            lora_suffix_B=lora_cfg.get("suffix_B", "lora_B"),
        )
        self._obj.node_var.model_weight = local_weight
