"""
rblasa (Semantic-Anchored Rank Alignment) client training strategy.
...
class rblasaClientTrainingStrategy(ClientStrategy):
    """
    Client strategy for rblasa federated LoRA training.

    Differs from plain RBLA by:
    1. Caching the global (A_g, B_g) anchor tensors received from the server.
    2. Injecting :class:`rblasaAlignmentLoss` into the trainer before each round
    ...
    def __init__(self, args, client_node):
        ...
        # ── rblasa alignment module (lazy-init from config) ──
        self._rblasa_alignment: rblasaAlignmentLoss | None = None
        self._rblasa_config: rblasaConfig | None = None
    ...
    # ------------------------------------------------------------------
    # Lazy rblasa config from node config dict
    # ------------------------------------------------------------------
    def _ensure_rblasa(self) -> rblasaAlignmentLoss:
        if self._rblasa_alignment is None:
            cfg_dict = self._obj.node_var.config_dict.get("rblasa", {})
            self._rblasa_config = rblasaConfig.from_dict(cfg_dict)
            self._rblasa_alignment = rblasaAlignmentLoss(self._rblasa_config)
        return self._rblasa_alignment
    ...
    def local_training_step(self) -> Tuple[dict, Any]:
        """Full local training with rblasa alignment."""
        node_vars: FedNodeVars = self._obj.node_var
        ...
        # ── Inject rblasa context ──
        self._inject_rblasa_context(tr)
    ...
    # ------------------------------------------------------------------
    # rblasa context injection
    # ------------------------------------------------------------------
    def _inject_rblasa_context(self, trainer) -> None:
        """Set global anchors, round index, and alignment module on the trainer."""
        if not hasattr(trainer, "set_rblasa_context"):
            return  # trainer doesn't support rblasa �?skip alignment

        rblasa = self._ensure_rblasa()
