...
class FedSDRServerStrategy(ServerStrategy):
    """Server-side strategy to wire FedSDR selector feedback each round."""

    def __init__(self, args, server_node) -> None:
        super().__init__()
        self._args = args
        self._strategy_type = "fedsdr"
    ...
    def receive_client_updates(self, client_updates: List[Dict[str, Any]]) -> None:
        """Pass training feedback to FedSDR selector via with_clients_data."""
        ...
        if selector and hasattr(selector, "with_clients_data"):
            ...
            for update in client_updates:
                # update = {"updated_weights": ..., "train_record": {...}, "latency": ...}
                # train_record wrapper = {"node_id": ..., "train_record": {...trainer stats...}, "data_sample_num": ...}
