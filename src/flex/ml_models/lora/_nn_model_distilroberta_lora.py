...
class NNModel_DistilRoBERTaLoRA(NNModel):
    """
    DistilRoBERTa-base with LoRA-enabled layers for sequence classification.

    DistilRoBERTa is a distilled version of RoBERTa-base that retains ~95%
    performance while being 40% smaller and 60% faster. It has 6 transformer
    layers (vs. 12 in RoBERTa-base / 24 in RoBERTa-large).

    Uses HuggingFace transformers to load the pretrained DistilRoBERTa checkpoint,
    ...
    Configurable via NNModelArgs:
        ...
        - lora_embedding: bool = False  (DistilRoBERTa embedding is usually not LoRA-tuned)
    ...
    def create_model(self, args: NNModelArgs) -> AbstractNNModel:
        ...
        # ---- 1. Basic config -------------------------------------------------
        model_name = args.get("pretrained_model", "distilroberta-base")
        ...
        if base_lora_r == 1 and rank_ratio != 1.0:
            console.warn(
                f"[DistilRoBERTa LoRA] rank_ratio={rank_ratio} is configured but base lora_r=1, "
        ...
        if self.pad_id is None:
            self.pad_id = int(args.get("pad_id", 1))  # DistilRoBERTa default pad_token_id = 1
    ...
    def _configure_trainable_parameters(self, train_classifier: bool = True) -> None:
        """
        Freeze the pretrained DistilRoBERTa base and train only LoRA adapters.
        ...
        DistilRoBERTa layer would otherwise remain trainable. That turns LoRA
        training into near full fine-tuning and creates large optimizer state.
        """
