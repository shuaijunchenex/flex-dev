...
class DatasetLoader_RTE(DatasetLoader):
    """Dataset loader for GLUE RTE.
    ...
    def _create_inner(self, args: DatasetLoaderArgs) -> None:
        ...
        dataset = (
            load_dataset("nyu-mll/glue", "rte", cache_dir=root)
            if is_download
            else load_dataset("nyu-mll/glue", "rte")
        )
        ...
        self.train_label_distribution = dict(
            DatasetLoaderUtil.count_label_distribution(self._dataset, "RTE-Train")
        )
        self.test_label_distribution = dict(
            DatasetLoaderUtil.count_label_distribution(self._test_dataset, "RTE-Validation")
        )
